...
> **Zone-governance ruling (2026-05-25 - #DZ-5392)** Yusuf: ledger-pressure zone-exposure amplifier, final. A drift window's containment backlog does not depend on its own volume alone: a window sitting on a high-exposure authoritative zone compounds the responder cost, so the ledger pressure now folds in the window's attributed `trust_exposure_score` — the node-disjoint delegation packing PLUS the contention credit, i.e. the exact value emitted in `trust_exposure_score`, not the bare packing — divided by 7 and FLOORED: `ledger_pressure_score = ceil(carry_out_ms / 80) + ceil(carry_in_ms / 120) + max(alert_count - 1, 0) + (trust_exposure_score // 7)`. The three earlier terms and their rounding (the 80 carry-out ceiling, the 120 carry-in ceiling, the alert term) are unchanged; this only appends the zone-exposure term. Because the amplifier reads the attributed exposure, an error in the delegation packing or its contention attribution propagates into the ledger pressure and thus the priority tier. ROUNDING: trust_exposure_score // 7 = FLOOR.
